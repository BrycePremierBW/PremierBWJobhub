...
- **PlanReader** gains a **Colour Schedule** step: it auto-builds a per-room /
  per-surface colour schedule from the take-off rows and detected rooms, lets
  you assign colour, finish, product and an optional hex swatch, and exports a
  print-ready schedule as Excel and PDF.
- **Colour markup**: with one click PlanReader renders **colour markup images**
  — each plan page with a colour chip card overlaid on every room marker,
  showing the room's surfaces and their assigned colours (with a colours-used
  legend). These are saved to the job and downloadable.
- **JobHub** gains a **Colour Schedule** section (available to managers and
  employees): pick a job and see its colour markup images, colour schedule
  tables and downloads, generated by PlanReader and attached to the job. The
  Employee Portal's "My Job Info" tab also shows the job's colour schedule.
