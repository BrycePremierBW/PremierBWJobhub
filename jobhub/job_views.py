...
def render_selectable_job_details(job_details, job_id):
    ...
    employee_names = [""] + list(get_employee_options(active_only=True).keys())
    ...
    with st.form(f"quick_job_details_form_{int(job_id)}"):
        ...
        new_builder = st.selectbox("Builder / Client", builder_names, index=builder_names.index(current_builder))
        new_site_address = st.text_input("Site Address", value=str(row.get("Site Address", "") or ""))
        c3, c4, c5 = st.columns(3)
        new_status = c3.selectbox("Status", status_options, index=status_options.index(current_status))
        new_leading_hand = c4.selectbox("Leading Hand", employee_names, index=employee_names.index(current_leading_hand))
        new_contract_value = c5.number_input("Contract Value Ex GST", min_value=0.0, step=100.0, value=float(row.get("Contract Value Ex GST", 0) or 0))
        c6, c7 = st.columns(2)
        new_start_date = c6.date_input("Start Date", value=job_date_value(row.get("Start Date")), format="DD/MM/YYYY")
        new_end_date = c7.date_input("End Date", value=job_date_value(row.get("End Date")), format="DD/MM/YYYY")
    ...
    if save_details:
        ...
        try:
            ...
            if builder_id:
                execute(
                    "UPDATE builders_clients SET contact_name = ?, phone = ?, email = ?, terms = ? WHERE id = ?",
