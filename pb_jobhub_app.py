...
def user_access_page():
    ...
    with tab_edit:
        ...
        if users_df.empty:
            ...
            st.warning(
                "This deletes the selected login account only. It does not delete the linked employee record, "
            ...
            if st.button("Delete Selected User Account", key=f"delete_user_button_{selected_user_id}"):
                if delete_confirm.strip().upper() != "DELETE USER":
                    ...
                    execute("DELETE FROM app_users WHERE id = ?", (selected_user_id,))
                    st.success(f"Deleted user account: {selected_username}")
                    refresh()
...
def timesheets_page(employee_restricted=False):
    ...
    with tab_review:
        ...
        if timesheets_df.empty:
            ...
            st.dataframe(timesheets_df.drop(columns=["id"]), use_container_width=True, hide_index=True)
    ...
    with tab_by_job:
        ...
        if not job_options:
            ...
            if by_job_df.empty:
                ...
                st.dataframe(by_job_df, use_container_width=True, hide_index=True)
...
def estimate_working_sheet_page():
    ...
    with tab_lines:
        ...
        if lines_df.empty:
            ...
            st.dataframe(lines_df.drop(columns=["id"]), use_container_width=True, hide_index=True)
    ...
    with tab_view:
        ...
        st.dataframe(lines_export, use_container_width=True, hide_index=True)
    ...
    with tab_job_pack:
        ...
        if not job_options:
            ...
            if estimate_summary.empty:
                ...
                st.dataframe(estimate_summary, use_container_width=True, hide_index=True)

            st.markdown("### Estimate Line Items for this Job")
            if estimate_lines.empty:
                st.info("No estimate line items saved for this job.")
            else:
                st.dataframe(estimate_lines, use_container_width=True, hide_index=True)
            ...
            if timesheet_details.empty:
                ...
                st.dataframe(timesheet_details, use_container_width=True, hide_index=True)
