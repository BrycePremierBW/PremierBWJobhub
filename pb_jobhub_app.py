...
def df_query(sql, params=()):
    if USE_POSTGRES:
        ...
        try:
            cur = raw_conn.cursor()
            cur.execute(adapt_sql_for_postgres(sql), params)
            rows = cur.fetchall()
    ...
    with tab_remove:
        ...
        if employees_df.empty:
            ...
            if col2.button("Delete Employee"):
                if has_related_records("wage_entries", "employee_id", selected_id) or has_related_records("timesheet_entries", "employee_id", selected_id):
                    execute("UPDATE employees SET status = 'Inactive' WHERE id = ?", (selected_id,))
