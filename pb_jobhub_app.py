...
import requests
import psycopg2
from psycopg2.pool import ThreadedConnedef generate_paint_order_pdf(job_id):ctionPool
...
def generate_paint_order_pdf(job_id):
    ...
    return output_path, variation_nojob = get_job_details_for_pdf(job_id)
   ...
   tab_jobs, tab_hours, tab_equipment, tab_forms, tab_photos, tab_password = st.tabs([
    ...
    with tab_forms:
        ...
        if not job_options:
            ...
            with st.form("employee_variation_form_generator"):
...
def render_job_linked_info(job_id, expanded=True):
    ...
    material_details = safe_df_query("""
        ...
        SELECT m.id AS "ID",
               ...
               m.qty_required AS "Qty Required",
               m.qty_received AS "Qty Received",
               ROUND(COALESCE(m.custom_unit_price, p.price_ex_gst, 0) * m.qty_required, 2) AS "Total Cost Ex GST",
...
# =============================
    ...
    df = df_query("""
        SELECT m.id AS 'ID',
               ...
               m.qty_required AS 'Qty Required',
               m.qty_received AS 'Qty Received',
               ROUND(COALESCE(m.custom_unit_price, p.price_ex_gst, 0) * m.qty_required, 2) AS 'Total Cost',
               m.date_ordered AS 'Date Ordered',
               m.notes AS 'Notes'
        FROM material_entries m
