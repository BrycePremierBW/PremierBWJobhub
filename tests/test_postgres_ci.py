...
checks that JobHub's enterprise schema can be created idempotently, that normal
read/write/update transactions work, and that a fresh physical connection can
resume work after the first connection is closed.
...
def main() -> int:
    ...
    try:
        ...
        # The production schema bootstrap must be safe to run repeatedly.
        ...
        try:
            ...
            cursor.execute(
                ...
                WHERE table_schema = ?
                  AND table_name IN ('app_error_events', 'backup_runs', 'field_forms')
                """,
                (schema,),
            )
